<h1 align="center" style="font-size:28px; line-height:1"><b>Budgy</b></h1>

<a href="https://github.com/jonahmvp/Budgy">
  <div align="center">
...
Budgy is a full-fledged, feature-rich application designed to empower users in managing their finances effectively. Built using Flutter - with Drift's SQL package, and Firebase - this app offers a seamless and intuitive user experience across various devices. Development started in September 2021.
...
This application may become availabe on app stores at a later date. See releases: [GitHub](https://github.com/jonahmvp/Budgy/releases).

### Changelog

Changes and progress about development is all heavily documented in GitHub [commits](https://github.com/jonahmvp/Budgy/commits/main/) and in the [changelog](https://github.com/jonahmvp/Budgy/blob/main/budget/lib/widgets/showChangelog.dart)
...
### Changelog
* Introduce iOS support (Budgy v5.3.5+400 and up)
* **Breaking:** Remove unsupported app link domain - `cashew://budget.app` (Budgy v5.3.4+396 and up)
* Support for more parameters documented under `Detailed Parameters` (Budgy v5.2.9+358 and up)
* Introduce Android and web support (Budgy v5.2.3+328 and up)

### Examples (for Android and iOS)

Ensure Budgy is installed on the device you are launching these URLs from.
...
| `JSON`        | A list of JSON objects of transactions. If provided, Budgy will import a list/multiple transactions at once. Each JSON object in the list can use any of the aforementioned parameters. The JSON object should be keyed with `transactions` followed by the list of objects. See the example below. | No       | None            |
...
| ----- | ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | --------------- | ---------- |
| a     | `messageToParse` | Uses a scanner template to parse the passed in message. Only `date` or `dateCreated` can be passed along with this standalone parameter. All other fields will be constructed by the scanner template. Scanner templates are not enabled in Budgy by default. To enable this feature, enable `Notification Transactions` debug feature. | None     | None            | Yes        |
...
#### Android
If you get redirected to the website instead of the Budgy app, your device may not be correctly set up to open App Links. Ensure Budgy can open its respective app link. On your Android device head to your device settings, 'Default apps', 'Opening links' and ensure Budgy has permission to open from the domain `cashewapp.web.app` under 'Supported web addresses'. This may be disabled by default if you did not install a Google Play signed copy of Budgy (i.e. downloaded from GitHub). This is because Budgy now uses App Links which need to be verified as per Android 12+ requirements with an app's signature public key. Alternatively, you can use a redirect application such as LinkSheet which can manage which apps open when certain links are activated.
...
You can click links and open them with Budgy. See the example section above to test.
...
Unfortunately, I am currently not accepting contributions due to licensing and credits. Since this application turns some profits, I want to avoid any muddy water when it comes to compensation for contributions. You are free to submit an [issue](https://github.com/jonahmvp/Budgy/issues) and I can consider it!
...
- https://github.com/jonahmvp/Budgy/releases/new
...
- Downloads the latest version of Budgy translations. Runs `budget\assets\translations\generate-translations.py`
